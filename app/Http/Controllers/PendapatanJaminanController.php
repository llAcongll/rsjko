...
class PendapatanJaminanController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->header('Accept') === 'application/json') {
            ...
            // Fetch deductions for JAMINAN category for the current year
    ...
    public function import(Request $request)
    {
        ...
        try {
            while (($row = fgetcsv($handle, 0, $delimiter)) !== false) {
                ...
                $ruanganId = $ruangans[$namaRuangan] ?? 39; // default 39 (Lain-lain)
    ...
    public function bulkDelete(Request $request)
    {
        ...
        $count = PendapatanJaminan::where('tanggal', $request->tanggal)
            ->where('tahun', session('tahun_anggaran'))
            ->delete();
        return response()->json(['success' => true, 'count' => $count]);
    }

...
class PendapatanKerjasamaController extends Controller
{
    ...
    {
        abort_unless(auth()->user()->hasPermission('PENDAPATAN_KERJA_MANAGE') || auth()->user()->isAdmin(), 403);
        ...
        $pendapatan = PendapatanKerjasama::create($data);

        ActivityLog::log(
            'CREATE',
            'PENDAPATAN_KERJA',
            "Menambah pendapatan kerjasama pasien {$pendapatan->nama_pasien}",
            $pendapatan->id,
            null,
            $pendapatan->toArray()
        );
    ...
    public function update(Request $request, $id)
    {
        ...
        ActivityLog::log(
            'UPDATE',
            'PENDAPATAN_KERJA',
            "Mengubah pendapatan kerjasama pasien {$pendapatan->nama_pasien}",
            $pendapatan->id,
            $oldValues,
            $pendapatan->toArray()
        );
    ...
    public function destroy($id)
    {
        ...
        ActivityLog::log(
            'DELETE',
            'PENDAPATAN_KERJA',
            "Menghapus pendapatan kerjasama pasien {$pendapatan->nama_pasien}",
            $id,
            $oldValues,
            null
        );
    ...
    public function downloadTemplate()
    {
        ...
        $callback = function () use ($columns) {
            ...
            fputcsv($file, ['2026-02-15', 'BUDI KERJASAMA', 'FISIOTERAPI', 'MOU PERUSAHAAN A', 'NON_TUNAI', 'BRK', 'TRANSFER', '300000', '150000', '200000', '75000']);
            fclose($file);
        };
    ...
    public function import(Request $request)
    {
        ...
        $count = 0;
        $this->service->transaction(function () use ($handle, $delimiter, $ruangans, $mous, &$count, $request) {
        ...
        ActivityLog::log('IMPORT', 'PENDAPATAN_KERJA', "Berhasil mengimpor {$count} data pendapatan kerjasama", null, null, null);
    ...
    public function bulkDelete(Request $request)
    {
        ...
        $query = PendapatanKerjasama::where('revenue_master_id', $request->revenue_master_id);
        $count = $query->count();
        $query->delete();

        RevenueMasterController::recalculate($request->revenue_master_id);

        ActivityLog::log('DELETE', 'PENDAPATAN_KERJA', "Menghapus massal {$count} data pendapatan kerjasama pada kelompok ID {$request->revenue_master_id}", null, null, null);
        return response()->json(['success' => true, 'count' => $count]);
    }
}

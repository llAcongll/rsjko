...
window.initPendapatanUmum = function () {
  ...
  if (form) {
    ...
    ['metode_pembayaran', 'bank', 'metode_detail'].forEach(name => {
      form.querySelector(`[name="${name}"]`)?.addEventListener('change', cekSiapSimpan);
    });
  }
...
function setupBankLogic() {
  ...
  metode.addEventListener('change', () => {
    ...
    if (metode.value === 'TUNAI') {
      ...
      detail.disabled = true; // Wait for bank to be chosen
...
window.exportSemuaPendapatanUmum = function (btn) {
  ...
  fetch(`/dashboard/pendapatan/umum?${params.toString()}`, {
    ...
    .then(res => {
      const data = res.data || [];
      let oldTable = document.getElementById('tempExportUmum');
      if (oldTable) oldTable.remove();

      const table = document.createElement('table');
      table.id = 'tempExportUmum';
      ...
      previewExportExcel('tempExportUmum', 'Data Laporan Pendapatan Umum - Semua');
